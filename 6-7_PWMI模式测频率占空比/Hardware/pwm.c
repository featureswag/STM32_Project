...
void PWM_Init(void)
{
    ...
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;/* GPIO_Mode_AF_PP is a mode 
                                                      used to configure a GPIO pin as an alternate function output with push-pull configuration.  */
...
void PWM_SetPrescaler(uint16_t Prescaler_Value)
{
   TIM_PrescalerConfig(TIM2, Prescaler_Value, TIM_PSCReloadMode_Immediate); 
}